...
func main() {
	...
	cadenaRecibida, _ := ioutil.ReadFile("prueba.txt")

	is := antlr.NewInputStream(string(cadenaRecibida))
	...
	for _, cons := range eval.ListConsole {
		fmt.Print(cons)
	}
	fmt.Println("")
	for _, env := range eval.Traductor.GenerateTraduction() {
		fmt.Print(env)
	}
