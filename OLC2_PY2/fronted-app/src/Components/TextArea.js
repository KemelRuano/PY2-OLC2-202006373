...
import iconconsole from '../Components/terminal.ico'
...
export function TerminalPrint({ console }) {
  return (
    <div>
          {console.map(item => { 
            return (
              <pre key={item.key} style={{ lineHeight: '0.95', margin: '0' ,fontSize: '16px'}}>{item.terminal}</pre>
            );
          })}
    </div>
  ...
  return (
    <div>
      <Tabs selectedIndex={activeTab} onSelect={handleTabSelect}>
        <TabList>
          <Tab> <img src={iconconsole} alt="icono" style={{maxWidth: '30px'}} /></Tab>
